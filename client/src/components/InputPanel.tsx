...
const InputPanel = ({ onGenerateBracket }: InputPanelProps) => {
  ...
  const handleGenerateBracket = () => {
    ...
    if (inputMethod === "bulk") {
      ...
      participantsList = participants.individual
    ...
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      <Tabs value={inputMethod} onValueChange={setInputMethod}>
        ...
        <TabsContent value="bulk" className="mb-6">
          <Label htmlFor="participants-bulk" className="block text-sm font-medium text-slate-700 mb-2">
          ...
          <div className="space-y-3">
            {participants.individual.map((participant, index) => (
              <div key={index} className="mb-3">
                <Label htmlFor={`participant-${index}`} className="block text-sm font-medium text-slate-700 mb-2">
                  Participant {index + 1}
                </Label>
                <Input
                  id={`participant-${index}`}
                  value={participant}
                  onChange={(e) => updateIndividualParticipant(index, e.target.value)}
                  className="w-full px-3 py-2 border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary text-sm"
                />
              </div>
            ))}
          ...
          <div className="flex items-center justify-center w-full">
            <Label htmlFor="file-upload" className="w-full cursor-pointer">
              <div className="border-2 border-dashed border-slate-300 rounded-md p-6 flex flex-col items-center justify-center hover:border-primary transition-colors">
                ...
                <p className="mt-2 text-sm text-slate-600 font-medium">Drop file here or click to upload</p>
                <p className="mt-1 text-xs text-slate-500">One participant name per line</p>
              </div>
              <Input 
                ref={fileInputRef}
                id="file-upload" 
      ...
      <div className="mb-4">
        ...
        >
          ...
          <div className="flex items-center">
            ...
            <Label htmlFor="as-entered" className="ml-2 text-sm text-slate-700">As Entered (No Seeding)</Label>
          </div>
        </RadioGroup>
      </div>
