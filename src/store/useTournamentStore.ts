...
import { exportBracketAsPNG, exportBracketAsPDF, copyBracketToClipboard } from '@/utils/exportUtils';
...
export const useTournamentStore = create<TournamentState>()(
  ...
  exportAsPNG: () => {
    const { closeExportModal, showToast } = get();
    exportBracketAsPNG(showToast, closeExportModal);
  },
  
  exportAsPDF: () => {
    const { closeExportModal, showToast } = get();
    exportBracketAsPDF(showToast, closeExportModal);
  },
  
  copyToClipboard: () => {
    const { closeExportModal, showToast } = get();
    copyBracketToClipboard(showToast, closeExportModal);
  },
